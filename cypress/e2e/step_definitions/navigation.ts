...
When('the user navigate the {string} header link', (linkText: string) => {
  ...
  cy.get('header').contains('a', linkText).invoke('removeAttr', 'target').click();
});
...
When('the user opens the user menu and navigate {string}', (menuItem: string) => {
  ...
  cy.get('.dropdown-menu').should('be.visible').contains(menuItem).click();
});
...
Then('the Update Password modal is visible', () => {
  cy.get('.modal-title').should('be.visible').and('contain.text', 'Update Password');
});
